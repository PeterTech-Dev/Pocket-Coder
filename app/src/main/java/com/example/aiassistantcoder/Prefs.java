...
public final class Prefs {
    ...
    public static float editorFontSize(Context c) {
        ...
        if (p.contains(KEY_EDITOR_FONT_SIZE)) {
            try {
                return p.getFloat(KEY_EDITOR_FONT_SIZE, 14f);
            } catch (ClassCastException ignored) {
            }
            try {
                return (float) p.getInt(KEY_EDITOR_FONT_SIZE, 14);
            } catch (ClassCastException ignored) {
            }
        }
        return 14f;
    ...
    public static float consoleFontSize(Context c) {
        ...
        if (p.contains(KEY_CONSOLE_FONT_SIZE)) {
            try {
                return p.getFloat(KEY_CONSOLE_FONT_SIZE, 13f);
            } catch (ClassCastException ignored) {
            }
            try {
                return (float) p.getInt(KEY_CONSOLE_FONT_SIZE, 13);
            } catch (ClassCastException ignored) {
            }
        }
        return 13f;
